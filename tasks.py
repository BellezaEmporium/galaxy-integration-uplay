...
def build(c, output='output', ziparchive=None):
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False) as tmp:
        c.run("pip-compile requirements/app.txt --resolver=backtracking --output-file=-", out_stream=tmp)
...
@task
def install(c):
    dist_path = os.path.join(DIST_DIR, "uplay_" + MANIFEST['guid'])
    build(c, output=dist_path)


@task
def pack(c):
    build(c, output="uplay_" + MANIFEST['guid'], ziparchive='uplay_v{}.zip'.format(MANIFEST['version']))
    print('--> Removing {} directory'.format("uplay_" + MANIFEST['guid']))
    rmtree("uplay_" + MANIFEST['guid'])
